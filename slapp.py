...
# st.title("🛠️ Machine Failure Prediction with Custom Priority Order")
    ...
    if search_term:
        ...
        if not search_cols:
            ...
            filtered_df = sorted_df[
                sorted_df[search_cols].apply(
                    lambda x: (x.astype(str) == search_term) |
                              (x == search_term_int if search_term_int is not None else False)
                ).any(axis=1)
            ]
